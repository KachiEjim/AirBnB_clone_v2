...
def python(text):
    """ Returns the string 'Python' followed by
    the modified `text` parameter, replacing underscores with spaces. """
    return 'Python {}'.format(text.replace("_", " "))


@app.route('/number/<int:n>', strict_slashes=False)
def number(n):
    """ Returns a string indicating that the provided
    integer `n` is a number. """
    return '{} is a number'.format(n)


@app.route('/number_odd_or_even/<int:n>', strict_slashes=False)
def number_odd_or_even(n):
    """ Renders a template that displays whether the number `n`
    is odd or even. """
    if n % 2 == 0:
        p = 'even'
    else:
